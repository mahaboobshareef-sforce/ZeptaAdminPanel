...
create or replace function public.is_delivery_agent() returns boolean language sql stable as $$ select public.current_user_role() = 'delivery_agent'; $$;
create or replace function public.is_customer() returns boolean language sql stable as $$ select public.current_user_role() = 'customer'; $$;

create or replace function public.is_order_owner(p_order uuid) returns boolean language sql security definer set search_path = public stable as $$
  select exists (select 1 from public.orders o where o.id = p_order and o.customer_id = auth.uid());
$$;

create or replace function public.is_order_agent(p_order uuid) returns boolean language sql security definer set search_path = public stable as $$
  select exists (select 1 from public.orders o where o.id = p_order and o.delivery_agent_id = auth.uid());
$$;
...
create policy tickets_customer_own on public.support_tickets for select using (customer_id = auth.uid());
create policy tickets_customer_insert on public.support_tickets for insert to authenticated with check (customer_id = auth.uid());
...
create policy messages_select on public.ticket_messages for select using (
  exists (select 1 from public.support_tickets st where st.id = ticket_id and (st.customer_id = auth.uid() or public.is_staff()))
);
...
create policy order_ratings_customer on public.order_ratings for insert to authenticated with check (customer_id = auth.uid());
